...
def find_no_verification(processed_file, file_to_process):
    file_to_fill = []
    processed_questions = []
    file_done_fill = []
...
prompt_dir = (
    "/mnt/pfs/zitao_team/tianqiaoliu/mathEval_data_check/matheval-lck/match_prompt"
)
system_input_info = "/mnt/pfs/zitao_team/tianqiaoliu/mathEval_data_check/matheval-lck/match_prompt/instruction.md"
...
if __name__ == "__main__":
    ...
    for one_ckpt_folder in os.listdir(this_model_data_save_dir):
        ...
        print(ckpt_result_folder)
        all_for_judge_files = glob.glob(
            os.path.join(ckpt_result_folder, "*extraction_for_judge.jsonl")
        )

        for one_jsonl_file in all_for_judge_files:
            with open(one_jsonl_file, "r") as f:
                json_content = f.readlines()
            base_json_file_name = os.path.basename(one_jsonl_file)
            ...
            #     # lines_to_fill, lines_done_fill = find_no_verification(
            #     #     verification_lines, json_content
            #     # )
            ...
            print(data_name)
            print(model_name)
            print(verification_output_file)
            print("No record for this")

            process_dataset(json_content, verification_output_file, n_jobs)
            print(
                "Done for model {} data {} partition {}".format(
                    model_name, data_name, base_json_file_name
                )
